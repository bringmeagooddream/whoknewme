...
from .utils import get_playlist_content
...
class IPTVChecker:
    ...
    async def run(self):
        ...
        playlist_content = await get_playlist_content(self.playlist_url)
        if not playlist_content:
            logger.warning(f"Could not load content from {self.playlist_url}, skipping.")
            return
