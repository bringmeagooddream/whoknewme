...
def read_playlist_urls(filepath):
    ...
    with open(filepath, 'r') as file:
        for line in file:
            ...
            url = url.strip()
            playlist_urls.append((country.strip(), url))
...
async def get_playlist_content(url_or_path):
    if url_or_path.startswith(('http://', 'https://')):
        return await download_playlist(url_or_path)
    else:
        try:
            if not os.path.isabs(url_or_path):
                base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                url_or_path = os.path.join(base_dir, url_or_path)
            with open(url_or_path, 'r', encoding='utf-8') as f:
                return f.read()
        except Exception as e:
            logger.error(f"Failed to read local playlist from {url_or_path}: {e}")
            return ""
